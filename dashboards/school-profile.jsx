// @ts-nocheck
// School profile - one institution: the headline numbers, what it offers by
// level, a pay-vs-debt scatter of ALL its reported programs (the fastest way to
// see whether a school is uniformly decent or wildly uneven), then the ranked
// list. jsx is sandboxed, so the kit is copied per dashboard on purpose.
...
export default function Dashboard({ dashboard, givens }) {
  const ink = useInk();
  const head = useQuery({ query: "school_headline", givens });
  const progs = useQuery({ query: "school_programs", givens });
  const lvls = useQuery({ query: "school_by_level", givens });

  const h = (head.rows || [])[0] || {};
  const rows = progs.rows || [];
  const lvlRows = lvls.rows || [];

  const pts = rows.map((r) => ({
    field: r.field, credential: r.credential,
    earnings: n(r.earnings), debt: n(r.debt),
    ratio: n(r.ratio), grads: n(r.grads), band: bandOf(n(r.ratio)),
  }));

  const lvlDb = [];
  lvlRows.forEach((r) => {
    lvlDb.push({ cat: r.credential, kind: "Programs", amount: n(r.programs_at_level), earnings: n(r.earnings), ratio: n(r.ratio) });
  });
  ...
  return (
    <PageShell dark={ink.dark}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 14, background: ink.surface, border: `1px solid ${ink.line}`, borderRadius: 10, padding: "14px 16px" }}>
        <Stat ink={ink} label="Programs reported" value={Math.round(n(h.programs_reported)).toLocaleString()} sub="with both figures" />
        <Stat ink={ink} label="Typical pay" value={usd(h.earnings)} sub="median, 1 year after" />
        <Stat ink={ink} label="Typical debt" value={usd(h.debt)} sub="median borrowed" />
        <Stat ink={ink} label="Debt vs pay" value={ratio(h.ratio)} sub="1.00x = one year's earnings" />
        <Stat ink={ink} label="Graduates / yr" value={Math.round(n(h.grads)).toLocaleString()} sub="across these programs" />
      </div>
          ...
          <div style={{ display: "grid", gap: 0 }}>
            ...
            {rows.slice(0, 15).map((r, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "22px 1fr auto", gap: 10, alignItems: "baseline", padding: "7px 0", borderTop: i ? `1px solid ${ink.line}` : "none" }}>
                ...
                >{r.field}<span style={{ color: ink.muted }}> · {r.credential}</span></div>
                <div style={{ textAlign: "right", whiteSpace: "nowrap" }}>
                  <div style={{ color: n(r.ratio) < 1 ? BAND.light : n(r.ratio) < 2 ? ink.text : BAND.vheavy, fontSize: 13, fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>{ratio(r.ratio)}</div>
                  <div style={{ color: ink.muted, fontSize: 11 }}>{usd(r.earnings)} pay · {usd(r.debt)} debt</div>
                </div>
              </div>
            ))}
          </div>
